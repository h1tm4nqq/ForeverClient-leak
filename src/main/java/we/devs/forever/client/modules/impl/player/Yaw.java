...
import we.devs.forever.api.util.client.Interpolation;
...
public class Yaw extends Module {
    private final Setting<Boolean> diagonal = new Setting<>("Diagonals", true);
    ...
    private final Setting<Float> interpSpeed = new Setting<>("Speed", 0.1f, 0f, 5f);

    private int ignoreTicks = 0;
    private float heightProgress = 0f;

    public Yaw() {
        super("Yaw", "Lock your yaw rotation", Category.PLAYER);
    }
            ...
            if (ignoreTicks <= 0) {
                ...
                if (entities.getValue()) {
                    if (mc.player.isRiding()) {
                        Objects.requireNonNull(mc.player.getRidingEntity()).prevRotationYaw = mc.player.getRidingEntity().rotationYaw;
