...
public class ToolTips
    ...
    public Setting<Boolean> maps = (new Setting<>("Maps", true));
    public Setting<Boolean> shulkers = (new Setting<>("ShulkerViewer", true));
    public Setting<Bind> peek = (new Setting<Bind>("Peek", new Bind(-1)));
    public Setting<Boolean> shulkerSpy = (new Setting<>("ShulkerSpy", true));
    public Setting<Boolean> render = (new Setting<>("Render", true, v -> this.shulkerSpy.getValue()));
    public Setting<Boolean> own = (new Setting<>("OwnShulker", true, v -> this.shulkerSpy.getValue()));
    public Setting<Integer> cooldown = (new Setting<>("ShowForS", Integer.valueOf(2), 0, Integer.valueOf(5), v -> this.shulkerSpy.getValue()));
    //private final Setting<Integer> red = (new Setting<>("Red", Integer.valueOf(255), 0, Integer.valueOf(255), v -> this.textColor.getValue()));
    //private final Setting<Integer> green = (new Setting<>("Green", 0, 0, Integer.valueOf(255), v -> this.textColor.getValue()));
    //private final Setting<Integer> blue = (new Setting<>("Blue", 0, 0, Integer.valueOf(255), v -> this.textColor.getValue()));
    //private final Setting<Integer> alpha = (new Setting<>("Alpha", Integer.valueOf(255), 0, Integer.valueOf(255), v -> this.textColor.getValue()));
    private final Setting<Color> color = (new Setting<>("Color", new Color(255, 255, 255, 255), ColorPickerButton.Mode.Normal, 100));
    public Setting<Boolean> offsets = (new Setting<>("Offsets", false));
    private final Setting<Integer> yPerPlayer = (new Setting<>("Y/Player", Integer.valueOf(18), v -> this.offsets.getValue()));
    private final Setting<Integer> xOffset = (new Setting<>("XOffset", Integer.valueOf(4), v -> this.offsets.getValue()));
    private final Setting<Integer> yOffset = (new Setting<>("YOffset", Integer.valueOf(2), v -> this.offsets.getValue()));
    private final Setting<Integer> trOffset = (new Setting<>("TROffset", Integer.valueOf(2), v -> this.offsets.getValue()));
    public Setting<Integer> invH = (new Setting<>("InvH", Integer.valueOf(3), v -> this.offsets.getValue()));
    ...
    public void renderShulkerToolTip(ItemStack stack, int x, int y, String name) {
        ...
        if (tagCompound != null && tagCompound.hasKey("BlockEntityTag", 10) && (blockEntityTag = tagCompound.getCompoundTag("BlockEntityTag")).hasKey("Items", 9)) {
            ...
            this.renderer.drawStringWithShadow(name == null ? stack.getDisplayName() : name, x + 8, y + 6, ColorUtil.toRGBA(color), false);
