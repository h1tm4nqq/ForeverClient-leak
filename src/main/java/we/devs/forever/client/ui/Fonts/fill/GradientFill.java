...
public class GradientFill implements Fill {
    ...
    private Setting<Color> startColSetting = null;
    private Setting<Color> endColSetting = null;
    ...
    public GradientFill(float x, float y, float width, float height, Setting<Color> start, Setting<Color> end) {
        this.start = new Vector2f(x, y);
        this.end = new Vector2f(x + width, y + height);
        this.startColSetting = start;
        this.endColSetting = end;
        this.startCol = start.getColor();
        this.endCol = end.getColor();
    }

    public GradientFill brighten(float amount) {
        if (startColSetting != null && endColSetting != null) {
            startCol = startColSetting.getColor();
            endCol = endColSetting.getColor();
        }
