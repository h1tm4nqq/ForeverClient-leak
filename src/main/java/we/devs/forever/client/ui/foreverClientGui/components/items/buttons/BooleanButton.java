...
public class BooleanButton extends Button {
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        anim.update();
        RenderUtil.drawRect(x, y, x + anim.getCurrentFloat(), y + height - 0.5f
                , getState()
                ...
                : !isHovering(mouseX, mouseY) ? 0x11555555 : 0x88555555);

        textManager.drawStringWithShadow(getName(), x + 2.3F, y - 1.65F - ForeverClientGui.getClickGui().getTextOffset(), getState() ? 0xFFFFFFFF : 0xFFAAAAAA);
        if (ClickGui.getInstance().desc.getValue() && this.isHovering(mouseX, mouseY)) {
            String description =
                    ChatFormatting.GREEN +
                            "Value " +
                            (setting.getValue() ? ChatFormatting.GREEN + "true" : ChatFormatting.RED + "false") +
                            "." +
                            "\n" +
                            ChatFormatting.WHITE +
                            (setting.getDescription().equals("") ? "A Boolean setting." : setting.getDescription());
